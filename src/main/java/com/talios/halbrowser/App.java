...
public class App {
    ...
    private static Stack<ReadableResource> resourceStack = new Stack<ReadableResource>();
    ...
    private static String contentType = ResourceFactory.HAL_XML;
    ...
    public static void main(String[] args) throws IOException, ExecutionException, InterruptedException {
        ...
        reader.setPrompt("\n> ");
        reader.addCompleter(new StringsCompleter("json", "xml", "auth", "open", "follow", "post", "back", "refresh", "quit", "history"));
//        reader.addCompleter(new RelsCompleter(resourceStack));
        reader.addCompleter(new RelsCompleter(resourceStack));
        reader.addCompleter(new UrlsCompleter(resourceStack));
        ...
        while ((input = reader.readLine().trim()) != null) {
            ...
            try {
                ...
                if ("xml".equals(command)) {
                    contentType = ResourceFactory.HAL_XML;
                    refresh();
                } else if ("json".equals(command)) {
                    contentType = ResourceFactory.HAL_JSON;
                    refresh();
                    ...
                    Optional<Link> link = resourceStack.peek().getLinkByRel(rel);
                    if (link.isPresent()) {
                    ...
                    postHref(resourceStack.peek().getResourceLink().getHref(), commandArgs);
                } else if ("back".equals(command)) {
                    resourceStack.pop();
                    reader.setPrompt("\n" + resourceStack.peek().getResourceLink().getHref() + "> ");
                    System.out.println(resourceStack.peek().renderContent(contentType));
    ...
    private static void refresh() throws IOException, InterruptedException, ExecutionException {
        ReadableResource resource = resourceStack.pop();
        followHref(resource.getResourceLink().getHref());
    }
    ...
    private static void followHref(String href) throws IOException, InterruptedException, ExecutionException {
        AsyncHttpClient.BoundRequestBuilder get = httpClient.prepareGet(href);
        if (auth != null && !"".equals(auth)) {
        ...
        if (response.getStatusCode() < 400) {
            ...
            ReadableResource resource = resourceFactory.readResource(new InputStreamReader(stream));

            if (isColorized) System.out.print(infoColor);
            System.out.println(resource.renderContent(contentType));
            if (isColorized) System.out.print(normalColor);
